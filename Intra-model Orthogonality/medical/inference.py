...
class _LoRA_qkv_IMO_add(nn.Module):
    """In Sam it is implemented as
    self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)
    B, N, C = x.shape
    qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
...
# checkpoint type six  ps:TC_IMO means LORA+IMO
parser.add_argument('--checkpoint_LORA', type=str, default='./work_dir_LORA_50/CT_Abd-Gallbladder/sam_model_best.pth')
parser.add_argument('--checkpoint_LORA_IMO', type=str, default='./work_dir_LORA_IMO_lr*100_linear/CT_Abd-Gallbladder/sam_model_best.pth')
...
parser.add_argument('--checkpoint_adapter_IMO', type=str, default='./work_dir_TAM_Adapter_IMO_lr*10/CT_Abd-Gallbladder/sam_model_best.pth')
...
if args.adaptation_type == 'scratch':
...
elif args.adaptation_type == 'LORA_IMO':
    sam_model_tune = sam_model_registry[args.model_type](checkpoint=args.checkpoint_initial).to(device)
    rank = 4
    t_len = 24
    attention_len = 2
    ...
    for blk in sam_model_tune.image_encoder.blocks:
        ...
        w_qkv_linear = blk.attn.qkv
        blk.attn.qkv = _LoRA_qkv_IMO_add(
            w_qkv_linear,
            w_a_linear_q,
            w_b_linear_q,
            w_a_linear_v,
        ...
        idx += attention_len
    sam_model_tune.load_state_dict(torch.load(args.checkpoint_LORA_IMO))
...
elif args.adaptation_type == 'Adapter_IMO':
    ...
    for blk in sam_model_tune.image_encoder.blocks:
        ...
        idx += 1
    sam_model_tune.load_state_dict(torch.load(args.checkpoint_adapter_IMO))
